...
export async function GET(req: NextRequest) {
  try {
    ...
    const backendResponse = await fetch(`${backendUrl}/api/videos/local-stream?${qs}`, {
      headers: {
    ...
    if (!backendResponse.ok || !backendResponse.body) {
      ...
      return new NextResponse(text || "Local video not available", { status: backendResponse.status });
    }
    ...
    console.error("local-stream proxy error:", error);
    return new NextResponse("Failed to load local video", { status: 500 });
  }
}
