...
/**
 * Same as 1st Month Savings: backend → n8n file-upload (Drive) → file_id.
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const fullUrl = `${backendUrl}/api/new-revenue/upload-pdf`;

    console.info(
      `${LOG_PREFIX} | request_id=${requestId} | invoice=${invoiceNumber} | stage=proxy_start | backend=${fullUrl}`
    );
    ...
    if (!backendResponse.ok) {
      ...
      console.error(
        `${LOG_PREFIX} FAIL | request_id=${parsed.request_id || requestId} | invoice=${invoiceNumber} | ` +
          `http=${backendResponse.status} | error_code=${parsed.error_code || "BACKEND_ERROR"} | ` +
    ...
    console.error(`${LOG_PREFIX} FAIL | request_id=${requestId} | stage=proxy_exception | message=${message}`);
    return NextResponse.json(
      { error_code: "INTERNAL", message, request_id: requestId },
      { status: 500 }
    );
  }
}
