...
const MEMBER_FIELDS: { key: string; label: string }[] = [
  ...
  { key: "nmis", label: "NMI(s)" },
];

const PERIOD_FIELDS: { key: string; label: string }[] = [
  { key: "start_date", label: "Start date" },
  { key: "end_date", label: "End date" },
  { key: "peak_rate_c_kwh", label: "Peak" },
  { key: "off_peak_rate_c_kwh", label: "Off-peak" },
  { key: "shoulder_rate_c_kwh", label: "Shoulder" },
  { key: "commission_c_kwh", label: "Commission" },
];

const CONDITION_FIELDS: { key: string; label: string }[] = [
  { key: "annual_kwh", label: "Annual consumption" },
  { key: "take_or_pay_pct", label: "Take or Pay" },
];
...
function AlintaElectricityAgreementRequestPageInner() {
  ...
  const prefillNmis = searchParams.get("nmis") ?? "";
  ...
  const subject = useMemo(() => {
    ...
    const nmis = fieldValue(draft, "nmis");
    const kind = draft.request_kind || "Retention";
    return `Agreement Request: E-C&I ${kind} ${company} NMI ${nmis}`.trim();
  }, [draft]);
  ...
  const handleExtract = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!file) {
      setError("Select the signed Alinta electricity engagement form PDF.");
      return;
    }
    if (!token) {
    ...
    try {
      ...
      fd.append("file", file);
      if (prefillNmis) fd.append("nmis", prefillNmis);
      const res = await fetch(`${getApiBaseUrl()}/api/alinta-electricity-agreement/extract`, {
        method: "POST",
    ...
    try {
      ...
      fd.append(
        ...
        JSON.stringify({
          ...
          nmis: fieldValue(draft, "nmis"),
        }),
      );
      const res = await fetch(`${getApiBaseUrl()}/api/alinta-electricity-agreement/send`, {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: fd,
      });
  ...
  const renderFields = (fields: { key: string; label: string }[]) => (
    <div className="grid gap-3 sm:grid-cols-2">
      {fields.map(({ key, label }) => {
        ...
        const wide = key === "address" || key === "nmis";
        return (
  ...
  return (
    <ToolPageLayout
      pageName="Send Alinta electricity agreement"
      title="Send Alinta electricity agreement"
      description="Upload a signed Alinta C&I electricity engagement form (scans are fine). We extract the details, look up NMI(s) on the signed C&I electricity sheet, then email the EF and LOA to data.quote."
      width="2xl"
    >
      {error ? (
        ...
        <CardContent className="space-y-4 pt-6">
          {step === 1 && (
            <form onSubmit={handleExtract} className="space-y-4">
              <div>
                <label className="mb-1 block text-sm font-medium">Signed Alinta electricity EF (PDF)</label>
                <input
                  type="file"
                ...
                {prefillNmis ? (
                  <p className="mt-2 text-xs text-gray-500">
                    Member NMIs on file: {prefillNmis}. These are used if the form does not list them.
                  </p>
                ) : null}
              </div>
          ...
          {step === 2 && draft && (
            <div className="space-y-5">
              <div className="flex flex-wrap items-center gap-2">
                ...
                >
                  {draft.request_kind}
                  {draft.match_kind && draft.match_kind !== "none" ? ` · ${draft.match_kind} NMI match` : " · no signed contract found"}
                </span>
                {draft.loa_available ? (
              ...
              <div className="grid gap-6 lg:grid-cols-2">
                <div className="space-y-5">
                  ...
                  <section>
                    <h3 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">Period & rates</h3>
                    {reviewEfLink ? (
                      <p className="-mt-2 mb-3 text-xs text-gray-500">
                        Check dates and rates against the form — {reviewEfLink}
                      </p>
                    ) : null}
                    {renderFields(PERIOD_FIELDS)}
                  </section>
                  <section>
                    <h3 className="mb-3 text-sm font-semibold text-gray-900 dark:text-white">Consumption</h3>
                ...
                <div className="space-y-3">
                  ...
                  <div className="rounded-xl border border-gray-200 bg-gray-50 p-4 text-sm dark:border-dark-3 dark:bg-dark-2">
                    ...
                    <div className="space-y-2 whitespace-pre-wrap text-gray-700 dark:text-gray-200">
                      ...
                      <p>
                        This is an Agreement Request for our member, {fieldValue(draft, "company_name")} (NMI{" "}
                        {fieldValue(draft, "nmis")}).
                      </p>
                      <p>
                      ...
                      <p>
                        ...
                        {"\n"}Peak: {fieldValue(draft, "peak_rate_c_kwh")}
                        {"\n"}Off-peak: {fieldValue(draft, "off_peak_rate_c_kwh")}
                        {fieldValue(draft, "shoulder_rate_c_kwh")
                          ? `\nShoulder: ${fieldValue(draft, "shoulder_rate_c_kwh")}`
                          : ""}
                        {"\n"}Commission: {fieldValue(draft, "commission_c_kwh")}
                      </p>
                      <p>Annual consumption: {fieldValue(draft, "annual_kwh")}</p>
                      <p>Take or Pay: {fieldValue(draft, "take_or_pay_pct")}</p>
...
export default function AlintaElectricityAgreementRequestPage() {
  return (
    <Suspense fallback={<div className="p-6 text-sm text-gray-500">Loading…</div>}>
      <AlintaElectricityAgreementRequestPageInner />
    </Suspense>
  );
}
