...
### CRM Model
Clients flow through stages: Lead → Qualified → LOA Signed → Data Collected → Analysis In Progress → Offer Sent → Won/Lost/Existing Client. Offers have separate statuses: Requested → Awaiting Response → Response Received → Autonomous Agent Trigger → Autonomous Agent Stopped → Accepted/Lost. The `/autonomous-agent` page lists offers in the two autonomous statuses (Running / Stopped tabs). All stage/status constants live in `src/constants/crm.ts`.
